      ...
      if (activeTab === 'login') {
        ...
        try {
          ...
          if (authErr.code === 'auth/email-already-in-use') {
            setErrorMsg("Account already exists in our security system. Please login instead.");
          } else if (authErr.code === 'auth/weak-password') {
            setErrorMsg("Validation Error: Password is too weak. Please use at least 6 characters.");
          } else {
            setErrorMsg(authErr.message || "Security registration failed.");
          }
          setIsSubmitting(false);
          return;
        }
        ...
        // Store donor user document in Firestore with role 'donor'
        const newDbUser = {
        ...
        <div className="p-6 md:p-10 flex flex-col justify-between">
          {showVerificationSent ? (
            ...
            >
              ...
              <h3 className="text-xl font-bold text-on-surface font-sans tracking-tight mb-2">
                Please check your inbox to verify your email.
              </h3>
              
              <p className="text-xs text-on-surface-variant max-w-sm mb-6 leading-relaxed">
                We have sent a secure verification link to <strong className="text-on-surface">{email.toLowerCase().trim()}</strong>. Please check your inbox (and spam folder) and verify your email to activate your donor dashboard access.
              </p>

              <div className="w-full bg-surface-container-low border border-outline-variant/30 rounded-xl p-4 mb-8 text-left">
                <h4 className="text-[10px] font-bold uppercase tracking-wider text-primary mb-2">What's Next?</h4>
                <ol className="list-decimal list-inside text-[11px] text-on-surface-variant space-y-1.5 leading-normal">
                  <li>Open the email from <strong>CareInventory AI</strong>.</li>
                  <li>Click on the unique verification link.</li>
              ...
              <button
                ...
                className="w-full bg-primary hover:bg-primary-container text-on-primary py-3.5 rounded-xl font-semibold text-xs transition-all shadow-sm active:scale-[0.98] flex items-center justify-center gap-2 uppercase tracking-wider cursor-pointer"
              >
